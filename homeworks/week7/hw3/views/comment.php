	...
	<?php 	// 如果登入，顯示留言表單
			if (isset($_COOKIE["cer_id"])) {
				...
				if ($result_user = $stmt_user->get_result()) {
					...
					setcookie("csrftoken", $csrftoken, 0, "/twgd");
		...
		$parent_id = $row["com_id"]; ?>
			...
			<div class="board__container">
				...
				<?php 	if ($row["time_edit"]) { ?>
							<div class="time time-edit">已編輯 <?= $row["time_edit"]?></div> 
				<?php	} ?>	
						</div>
					</div>				
...
<?php	// 讀取 comments 第二層 (回覆)
	...
	<?php	} ?>	<div class="status">
				...
				<?php 	if ($row_rpy["time_edit"]) { ?>
							<div class="time time-edit">已編輯 <?= $row_rpy["time_edit"]?></div> 
				<?php	} ?>						
						</div>					
					</div>
...
<?php	} ?>
	...
	<?php 	// 如果登入，顯示回覆表單
			if (isset($row_user)) { ?>
					...
					<div class="hidden user_id"><?= $row_user["user_id"] ?></div>
					<div class="hidden com_id"><?= $row["com_id"] ?></div>
<?php
			} else { ?>
					<div class="redirection">
