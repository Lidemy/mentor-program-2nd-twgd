...
$('.container').on('submit', 'form[name="comment_add"]', e =>{
	...
	$.ajax({
		...
		success: resp => {
			...
			if (res.parent_id === 0) {							
				// 顯示第一層 Comment
				showNewComment(el, res.user_id, res.username, res.com_id, res.time, res.content);
			} else {
				// 顯示第二層 Reply
				showNewReply(el, res.username, res.com_id, res.time, res.content);
				// 原 PO 回覆要顯示不同背景
...
const showNewComment = (element, user_id, username, com_id, time, content) => {
...
const showNewReply = (element, username, com_id, time, content) => {
	const newReplyHtml = `
		<div class="reply">
			<div class="status">
				<div>
	...
	$.ajax({
		...
		success: resp => {
			...
			const timeEdit = `
				...
				<div class="time time-edit">已編輯 ${res.time_edit}</div>`;
