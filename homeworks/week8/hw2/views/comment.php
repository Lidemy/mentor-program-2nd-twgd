...
<?php
	...
	// 讀取 comments 第一層
	$sql = "SELECT * FROM `twgd_comments` INNER JOIN `twgd_users` ON twgd_comments.user_id = twgd_users.user_id WHERE `parent_id`=0 ORDER BY created_at DESC"; 
...
<?php	// 讀取 comments 第二層 (回覆)
		$sql_rpy = "SELECT * FROM `twgd_comments` INNER JOIN `twgd_users` ON twgd_comments.user_id = twgd_users.user_id WHERE `parent_id` = ? ORDER BY created_at"; 
